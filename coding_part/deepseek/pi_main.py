...
def process_camera_frame(frame):
    ...
    try:
        ...
        # Create masks for track and walls
        ...
        # Morphological operations for signs
...
def update_lap_count(gyro_z_rate):
    ...
    try:
        ...
        # Integrate over the window to get angle change
        dt = 0.1
        ...
        # Reset turn count if angle is small
        ...
        if turn_count >= REQUIRED_TURNS_PER_LAP:
            ...
            # If three laps completed, transition to parking
    ...
    try:
        while True:
                ...
                if ser and ser.in_waiting > 0:
                    ...
                    if line:
                        ...
                        if line == "START":
                            ...
                            # Update lap count only if driving
                            if state == STATE_DRIVING_LAPS:
            ...
            if state == STATE_WAITING_FOR_START:
                ...
                # Process the frame to get error, sign info, and parking info
                error, sign_info, parking_lot_info = process_camera_frame(frame)

                # Check if we have completed 3 laps
                if lap_count >= REQUIRED_LAPS:
                    logger.info("Three laps completed. Initiating Parking sequence.")
                    state = STATE_PARKING
                    parking_sequence_step = 0
                    lap_count = 0  # Reset for potential reuse

                # Calculate motor commands based on current state
                ...
                # Small delay to prevent overwhelming the Nano
                time.sleep(0.05)

            elif state == STATE_PARKING:
                # Calculate parking commands
                dummy_error = 0
                ...
                # Wait for the parking step to complete
                time.sleep(1)
